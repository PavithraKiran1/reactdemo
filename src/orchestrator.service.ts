...
export class OrchestratorService implements OnModuleInit, OnModuleDestroy {
  ...
  async runAgentLoop(goal: string): Promise<{ goal: string; steps: OrchestratorStep[]; final: unknown }> {
    ...
    // If math tools exist, keep deterministic fallback; otherwise skip safely.
    const deterministic = await this.tryDeterministicPercentSumMultiply(goal, steps);
    if (deterministic) {
      return { goal, steps: deterministic.steps, final: deterministic.final };
    }
  ...
  private ensureObjectSchema(schema: unknown): Record<string, unknown> {
    // MCP listTools returns JSON Schema. If a server returns something else, be defensive.
  ...
  private async createOpenAiResponse(payload: Record<string, unknown>): Promise<OpenAiResponse> {
    ...
    const res = await fetch(`${baseUrl}/responses`, {
      ...
      body: JSON.stringify({
        model,
        instructions:
          'You are an orchestration agent. Use tools when you need to fetch order/journey data or perform calculations. For arithmetic or percentage calculations, you MUST use tools (no mental math). Return a concise final answer.',
        ...payload,
      }),
    });
  ...
  private needsMoreTools(text: string): boolean {
    ...
    const hasMathWords = /\b(add|subtract|multiply|divide|percent|percentage|times|sum|total|plus|minus|over)\b/i.test(
      text,
    );
    return hasDigits && hasMathWords;
  }

  private async getToolNames(): Promise<Set<string>> {
    const result = await this.listTools();
    const tools = (result as { tools?: Array<Record<string, unknown>> }).tools ?? [];
    return new Set(tools.map((t) => String(t.name)));
  }
  ...
  private async tryDeterministicPercentSumMultiply(
    ...
    const requiredMathTools = ['add', 'multiply', 'percent'];
    if (!requiredMathTools.every((t) => toolNames.has(t))) {
      return null;
    }

    const wantsDivide = /\b(divide|divided|devided|over)\b/i.test(normalized);
    if (wantsDivide && !toolNames.has('divide')) {
      return null;
    }
    ...
    steps.push({ action: 'call-tool', tool: 'multiply', args: { a: sum, b: multiplier }, result: multiplyResult });
    ...
    if (wantsDivide) {
      ...
      steps.push({ action: 'call-tool', tool: 'divide', args: { a: product, b: divisor }, result: divideResult });
    ...
    steps.push({ action: 'call-tool', tool: 'percent', args: { percent, value: baseValue }, result: percentResult });
    ...
    return { steps, final: `Final answer: ${finalNumber}` };
  }
